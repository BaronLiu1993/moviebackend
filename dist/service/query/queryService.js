...
export const getInitialFeed = async ({ supabaseClient, userId, }) => {
    try {
        ...
        const standardizedPopular = (popularData.results || []).map((item) => ({
            tmdb_id: item.id,
            title: item.name,
            release_year: item.first_air_date?.split('-')[0] || null,
            tags: item.genre_ids || []
        }));
        const standardizedAiring = (airingData.results || []).map((item) => ({
            tmdb_id: item.id,
            title: item.name,
            release_year: item.first_air_date?.split('-')[0] || null,
            tags: item.genre_ids || []
...
export const getFriendFilms = async ({ supabaseClient, userId, }) => {
    try {
...
export const getFriendsDramas = async ({ supabaseClient, userId, }) => {
    try {
